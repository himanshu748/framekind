...
Running that sweep produced a result that contradicts the standard advice for shrinking browser models. Quantization by itself bought nothing: UINT8 weights are 64% smaller, and the median inference was 3,183 ms against the FP32 reference's 3,154 ms, a ratio of 0.99. The backend was the real lever: the same FP32 graph moved to WebGPU ran 2.10× faster and reproduced the reference detections exactly. And quantization on WebGPU was actively harmful: WebGPU UINT8 ran slower than WebGPU FP32 and scored 0% agreement, so the smallest file was the worst configuration on the fastest backend. The largest single win was not precision at all. Halving the input resolution, one property on the image processor, was worth 7.94×.

There is an Arm-specific reason for each of those three results, worked through in `docs/arm-optimization.md` against a cost model that ships as tested code. YOLOS Tiny is a plain vision transformer, so at native resolution attention accounts for 56% of its multiply-accumulates and grows quadratically in token count. Halving the shortest edge cuts tokens by 3.31× but cuts the attention term by 10.93×, which is why resolution is the strongest lever. Quantization, meanwhile, cannot pay off in a browser at all: WebAssembly's `simd128` has no 8-bit dot product and no 8-bit matrix-multiply instruction, so the `SDOT` and `SMMLA` instructions that make INT8 fast on Arm, and that Arm exposes to native frameworks through KleidiAI, are unreachable from the wasm sandbox. Wiring quantized GEMM to them through WebAssembly Relaxed SIMD is still an open feature request against ONNX Runtime. The measured 0.99× is therefore the expected result, not an anomaly, and it predicts which Arm cores would benefit first if that work lands.
...
The first version of this project reported a large speedup from quantization measured in one browser. Building the sweep showed that the result did not hold: on a different browser the same comparison came out at 0.99×, and the apparent win had been a property of one runtime rather than of the optimization. Rebuilding the benchmark as a matrix, and adding a reference-based agreement score, is what turned a misleading number into a usable finding.
...
"Now the interesting part. This sweep runs every backend and precision this browser supports, against the same image, and scores each one against the full-precision reference. Quantization on its own bought nothing here, 0.99 times. Moving the same graph to WebGPU was worth 2.1 times with identical detections. And quantizing on WebGPU scored zero percent agreement, so the smallest file was the worst configuration."

**1:05 to 1:20, the consequence**
"There is an Arm reason for that. WebAssembly has no 8-bit dot product instruction, so the SDOT and SMMLA units that make INT8 fast on Arm are unreachable from the browser sandbox. The levers that do work are the GPU, and the token count: attention is 56 percent of this model's arithmetic, so halving the input resolution was worth nearly eight times. FrameKind runs the fastest configuration that still reproduces the reference exactly, and you can export every raw run as JSON."
